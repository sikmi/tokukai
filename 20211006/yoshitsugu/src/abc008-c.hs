...
solve :: [Int] -> Float
solve vs = (fromIntegral . sum . map calc $ permutations vs) / fromIntegral (traceShowId $ length vs)
  where
    calc :: [Int] -> Int
    calc as = let bs =  map (, True) as in calc1 bs 0

    calc1 :: [(Int, Bool)] -> Int -> Int
    calc1 [] _ = 0
    calc1 as n
      | n >= length as = sum . map fst $ filter snd as
      | otherwise =
        let a = trace ("a" ++ show (as !! n)) as !! n
            as' = take n as ++ map (\(a', b) -> if a' `mod` n == 0 then (a', not b) else (a', b)) (drop n as)
          in calc1 (traceShowId as') (n + 1)
