...
tribonacciModN :: Int -> [Int]
tribonacciModN m = 0 : 0 : 1 : [g t | t <- [3 ..]]
  where
    g n = (tribonacciModN m !! (n - 1) + tribonacciModN m !! (n - 2) + tribonacciModN m !! (n - 3)) `mod` m

main :: IO ()
main = do
  n <- fst . fromJust . runParser int <$> C.getLine
  print $ tribonacciModN 10007 !! (n - 1)
